...
def stHarmonic(frame, fs):
    ...
    """
    M = int(numpy.round(0.016 * fs) - 1)  # zjx add
...
def mfccInitFilterBanks(fs, nfft):
    ...
    fbank = numpy.zeros((nFiltTotal, int(nfft)))# zjx:nfft))
...
def stFeatureSpeed(signal, fs, win, step):
    ...
    [fbank, freqs] = mfccInitFilterBanks(fs, nfft) # zjx:, lowfreq, linsc, logsc, nlinfil, nlogfil)
    ...
    while (cur_p + win - 1 < N):
        count_fr += 1
        # print('===>>', win, cur_p) # zjx add
        # cur_p = int(cur_p)  # zjx add
        x = signal[cur_p:cur_p + win]
        cur_p = int(cur_p + step)
        X = abs(fft(x))
        X = X[0:nfft]
        X = X / len(X)
        Ex = 0.0
